...
# KNOWN-WORKING VENUS SEQUENCE PATH:
# Keep channel liquid-handling on named VENUS sequences. Do not convert tip racks
# to ad-hoc labwarePositions; VENUS uses the sequence/labware definition to retain
# required tip-rack properties such as MlStarTipRack.
...
def cross_match_positions(
    ...
    if ordered_steps:
        ...
        if logical_order:
            # A usable chronological source -> destination map is more valuable
            # than failing at the first replicate-count mismatch.  Replicate,
            # well, plate, labware and sample-order validation are performed
            # downstream for *every* logical sample.  This lets the analyzer
            # report all defects from a bad run instead of stopping after the
            # first source that received 1 dispense when 3 were expected.
            result = {}
...
def validate_tracking_source_order(
    aspirate_to_dispense_map: Dict[Any, Any],
    marker: str,
    layout_matches: Optional[Dict[str, List[Dict[str, Any]]]] = None,
    configured_replicate: str = "",
) -> Tuple[List[Dict[str, Any]], List[ValidationIssue]]:
    """
    Tracking-enabled validation ties the chronological source sample order in
    the trace to the numbered plate-layout markers.  It intentionally does not
    assume that a source well name such as B1 *is* sample X1; rather, the first
    distinct aspirated source is logical sample X1, the second X2, etc., and we
    verify that its actual destination(s) match that marker's expected wells.
    """
    findings: List[Dict[str, Any]] = []
    issues: List[ValidationIssue] = []

    prefix = str(marker or "").strip().upper()
    templates: List[Dict[str, Any]] = []
    samples_per_plate = 0
    expected_replicate_count = REPLICATE_MAP.get(
        str(configured_replicate or "").lower(), 1
    )
    ...
    if layout_matches:
        try:
            ...
            if templates:
                samples_per_plate = int(templates[0].get("samples_per_plate") or 0)
                expected_replicate_count = int(
                    templates[0].get("replicate_count") or expected_replicate_count
                )
        except Exception:
            # Main layout validation will report malformed layout configuration.
            templates = []

    actual_sample_count = len(aspirate_to_dispense_map)
    actual_total_dispenses = sum(
        len(dispenses or []) for dispenses in aspirate_to_dispense_map.values()
    )

    # For a single uploaded physical-plate template, tracking expects one set of
    # numbered samples per destination plate.  If the trace has more logical
    # source samples than the template but still uses one physical destination
    # plate, downstream multi-plate validation will independently flag that too.
    if samples_per_plate and actual_sample_count != samples_per_plate:
        finding = {
            "type": "tracking_sample_count",
            "status": "fail",
            "expected_samples_per_plate": samples_per_plate,
            "actual_logical_source_samples": actual_sample_count,
            "marker_prefix": prefix,
        }
        findings.append(finding)
        issues.append(ValidationIssue(
            "critical",
            "tracking sample count",
            f"Tracking expected {samples_per_plate} numbered sample(s) from the plate layout, "
            f"but the trace processed {actual_sample_count} distinct source sample(s).",
            finding,
        ))

    replicate_mismatches: List[Dict[str, Any]] = []
    ...
    for expected_order, (aspirate_step, dispenses) in enumerate(
        ...
        source_labware = (
            str(aspirate_step[1]).strip() if len(aspirate_step) > 1 else ""
        )
        source_position = (
            normalize_position_text(aspirate_step[2]) if len(aspirate_step) > 2 else ""
        )
        actual_destinations = [
            normalize_position_text(d[1])
            for d in (dispenses or [])
            if len(d) > 1 and str(d[1]).strip()
        ]
        actual_labwares = []
        for d in (dispenses or []):
            if len(d) > 1:
                lw = str(d[0]).strip()
                if lw and lw not in actual_labwares:
                    actual_labwares.append(lw)

        expected_wells: List[str] = []
        template_marker = f"{prefix}{expected_order}" if prefix else str(expected_order)
        plate_cycle = 1
        if layout_matches:
            try:
                expected = expected_plate_layout_for_sample(
                ...
                template_marker = str(expected.get("template_marker") or template_marker)
                plate_cycle = int(expected.get("plate_cycle") or 1)
                expected_wells = [
                    normalize_position_text(x)
                    for x in expected.get("expected_wells", [])
                ]
            except Exception:
                expected_wells = []

        replicate_ok = len(actual_destinations) == expected_replicate_count
        position_ok = (
            set(actual_destinations) == set(expected_wells)
            and len(actual_destinations) == len(expected_wells)
            if expected_wells else True
        )
        status = "pass" if replicate_ok and position_ok else "fail"

        finding = {
            "type": "tracking_source_order",
            "marker": f"{prefix}{expected_order}" if prefix else template_marker,
            "template_marker": template_marker,
            "expected_order": expected_order,
            "plate_cycle": plate_cycle,
            "source_labware": source_labware,
            "source_position": source_position,
            "dispense_count": len(actual_destinations),
            "expected_replicate_count": expected_replicate_count,
            "expected_destinations": expected_wells,
            "actual_destinations": actual_destinations,
            "actual_destination_labware": actual_labwares,
            "status": status,
        }
        findings.append(finding)

        if not replicate_ok:
            replicate_mismatches.append(finding)

        if expected_wells and not position_ok:
            issues.append(ValidationIssue(
                "critical",
                "tracking sample order",
                f"Tracked sample {expected_order} ({template_marker}) came from "
                f"{source_labware} {source_position} but was dispensed to "
                f"{', '.join(actual_destinations) or 'no destination'}; "
                f"the plate layout requires {', '.join(expected_wells)}.",
                finding,
            ))

    if replicate_mismatches:
        examples = []
        for finding in replicate_mismatches[:4]:
            examples.append(
                f"{finding.get('template_marker')}: {finding.get('dispense_count')} actual"
            )
        evidence = {
            "type": "tracking_replicate_check",
            "status": "fail",
            "expected_replicate_count": expected_replicate_count,
            "mismatch_count": len(replicate_mismatches),
            "actual_total_dispenses": actual_total_dispenses,
            "examples": examples,
        }
        ...
        issues.append(ValidationIssue(
            "critical",
            "replicate mismatch",
            f"Plate layout requires {expected_replicate_count} dispense(s) per sample "
            f"({configured_replicate or REPLICATE_NAME_BY_COUNT.get(expected_replicate_count, '')}), "
            f"but {len(replicate_mismatches)} tracked sample(s) had a different dispense count. "
            ...
            try:
                ...
                if ok and isinstance(cross_match, dict):
                    if marker_matches and pattern is not None:
                        ...
                        if t_issues:
                            for _summary in trace_findings:
                                if _summary.get("type") == "trace_summary":
                                    _summary["cross_match_status"] = "fail"
                                    break
