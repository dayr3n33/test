...
class DevPalLiteApp:
    ...
    def upload_layout(self):
        ...
        try:
            marker_matches = parse_plate_layout_file(path, "")
            ...
            if templates:
                detected_replicate = templates[0].get("replicate", "")
                if detected_replicate in REPLICATE_OPTIONS:
            ...
            # Keep the uploaded file. The Marker/Replicate UI fields remain a
            # fallback when auto-detection genuinely cannot determine them.
            self.status.config(
                text=f"Plate layout loaded; auto-detection warning: {exc}"
            )
