...
def build_report_sections(output: DevPalLiteOutput, report_path: str) -> Dict[str, Any]:
    ...
    # Keep the richer Version-2 marker review that explicitly states which
    # marker is present in every expected destination well.
    marker_summary = []
    for marker_label, entries in sorted(
        ...
        if wells:
            marker_summary.append(
                f"{marker_label}: "
                + ", ".join(f"{well} contains {marker_label}" for well in wells)
            )
