...
def ph_tip_pick_up_seq(ham_int, tipseq: str, channel: str):
    ...
    func = _get_optional_pyhamilton_function("tip_pick_up_seq", "tip_pickup_seq", "tip_pick_up", "tip_pickup")
    if func:
        return _call_best_effort(func, ham_int, tipseq=tipseq, tip_seq=tipseq, sequence=tipseq, channel=channel, channels=channel)
    return _send_command_best_effort(ham_int, "tip_pick_up_seq", tipseq=tipseq, tip_seq=tipseq, sequence=tipseq, channel=channel, channels=channel)
...
def ph_aspirate_seq(ham_int, asp_seq: str, vols: float, channel: str, liq_class: str):
    ...
    return _send_command_best_effort(ham_int, "aspirate_seq", asp_seq=asp_seq, sequence=asp_seq, vols=vols, volume=vols, channel=channel, channels=channel, liq_class=liq_class, liquid_class=liq_class)
...
def ph_dispense_seq(ham_int, disp_seq: str, vols: float, channel: str, liq_class: str):
    ...
    return _send_command_best_effort(ham_int, "dispense_seq", disp_seq=disp_seq, sequence=disp_seq, vols=vols, volume=vols, channel=channel, channels=channel, liq_class=liq_class, liquid_class=liq_class)
...
def ph_tip_eject_seq2(ham_int, waste_seq: str, channel: str):
    ...
    return _send_command_best_effort(ham_int, "tip_eject_seq2", waste_seq=waste_seq, sequence=waste_seq, channel=channel, channels=channel)
...
def get_layout_marker_positions(
    ...
    return [
        ...
        for entry in layout_matches.get(str(marker_label).upper().strip(), [])
...
def analyze_plate_layout_pattern(
    ...
    Infer:
      * marker prefix, e.g. S
      * marker numbers, e.g. 1..10
      * samples per plate = unique marker count (10)
      * replicate count = occurrences of each marker within ONE template
    """
    ...
    numbers = get_marker_numbers(layout_matches, prefix)
    ...
    if numbers != expected:
        ...
        raise ValueError(
            f"Marker numbering must be continuous from {prefix}1 through "
            f"{prefix}{max(numbers)}. Missing: {missing}."
        )
    ...
    for number in numbers:
        label = f"{prefix}{number}"
        entries = layout_matches.get(label, [])

        by_source = defaultdict(list)
        for entry in entries:
            by_source[str(entry.get("layout_source") or "default")].append(entry)

        if not by_source:
            raise ValueError(f"{label} has no destination well.")

        # Separate worksheets/tables can repeat an entire template. They are
        # not treated as replicate wells. Use the first template source for
        # positional validation after confirming occurrence counts agree.
        source_counts = {name: len(values) for name, values in by_source.items()}
        if len(set(source_counts.values())) != 1:
            raise ValueError(f"{label} has inconsistent occurrence count across layout sources: {source_counts}.")

        first_source = next(iter(by_source))
        selected_entries = by_source[first_source]

        wells = [
            normalize_position_text(entry["dest_value"])
            for entry in selected_entries
            if entry.get("dest_value")
        ]

        wells_by_number[number] = wells
        occurrences_by_number[number] = len(wells)

    replicate_counts = sorted(set(occurrences_by_number.values()))
    if len(replicate_counts) != 1:
        raise ValueError(
            "Every numbered marker must have the same replicate count. "
            f"Detected: {occurrences_by_number}."
        )

    replicate_count = replicate_counts[0]
    if replicate_count not in (1, 2, 3):
        raise ValueError(
            f"Layout indicates {replicate_count} destination(s) per sample; "
            "only single, duplicate, and triplicate are supported."
        )
    ...
    if configured_count is not None and configured_count != replicate_count:
        raise ValueError(
            f"Replicate mismatch: UI is '{configured_replicate}' ({configured_count}) "
            f"but plate layout indicates {replicate_count}."
        )
...
def resolve_repeating_layout_marker(
    ...
    pattern = analyze_plate_layout_pattern(layout_matches, marker)
    source_number = max(int(source_number), 1)
    marker_number = ((source_number - 1) % pattern.samples_per_plate) + 1
    return f"{pattern.marker_prefix}{marker_number}", marker_number
...
def expected_plate_layout_for_sample(
    ...
    marker_number = ((sample_number - 1) % pattern.samples_per_plate) + 1
    plate_cycle = ((sample_number - 1) // pattern.samples_per_plate) + 1
    return {
        "sample_number": sample_number,
        "template_marker": f"{pattern.marker_prefix}{marker_number}",
        "marker_number": marker_number,
        "plate_cycle": plate_cycle,
        "expected_wells": list(pattern.wells_by_marker_number[marker_number]),
...
def cross_match_positions(
    ...
    """
    Reused channels are preserved chronologically. This fixes the previous
    channel->last aspirate overwrite bug.
    """
    ...
    for channel, asp_records in asp_by_channel.items():
        ...
        expected_count = len(asp_records) * replicate_count

        if len(channel_dispenses) != expected_count:
            return False, (
                f"Channel {channel}: {len(asp_records)} aspirated sample(s), "
                f"{len(channel_dispenses)} dispense(s); {expected_count} expected "
                f"for {replicate_value}."
            )

        for occurrence, (global_index, asp) in enumerate(asp_records):
...
def validate_trace_labware_against_layout_format(
    ...
    expected_format = summarize_layout_format(layout_matches).get("inferred_plate_format", "unknown")
    findings, issues = [], []
    ...
    for dispenses in aspirate_to_dispense_map.values():
        for dsp in dispenses:
            if len(dsp) < 2:
                continue
            labware, position = str(dsp[0]).strip(), normalize_position_text(dsp[1])
            fmt = infer_labware_format_from_name(labware)
            status = "pass"

            if expected_format.startswith("unknown"):
                status = "warning"
            elif fmt not in {"unknown", "plate/unknown format", expected_format}:
                status = "fail"
                issues.append(ValidationIssue(
                    "critical",
                    "URGENT_LABWARE_MISMATCH",
                    f"Destination labware '{labware}' appears to be {fmt}, "
                    f"but layout appears to be {expected_format}.",
                    {"labware": labware, "destination": position},
                ))
...
def validate_trace_against_layout(
    ...
    """
    Validate an unlimited number of samples against a repeating plate template.

    Example:
      S1..S10 template, single replicate
      samples 1..10 -> physical plate cycle 1
      samples 11..20 -> physical plate cycle 2
      etc.

    A coordinate such as A1 may validly recur on a later physical plate.
    """
    ...
    for sample_index, (aspirate_step, dispenses) in enumerate(
        ...
        source_position = (
            normalize_position_text(aspirate_step[2])
        ...
        if missing or unexpected:
            reasons.append(
                f"expected well(s) {expected_positions}, "
                f"trace showed {actual_positions}"
            )
            ...
            if required_labware is None:
                ...
                if prior_cycle is not None and prior_cycle != plate_cycle:
                    reasons.append(
                        ...
                        f"'{actual_labware}' already assigned to plate cycle "
                        f"{prior_cycle}"
                    )
                else:
                ...
                reasons.append(
                    f"plate cycle {plate_cycle} must remain on destination "
                    f"labware '{required_labware}', but this sample used "
                    f"'{actual_labware}'"
                )
        ...
        finding = {
            ...
            "reason": (
                "; ".join(reasons)
                if reasons
                else "Plate-layout crossmatch passed."
            ),
        }
        ...
        if reasons:
            add_issue(
                ...
                (
                    f"{marker_label} (plate cycle {plate_cycle}, "
                    f"template {template_marker}) failed plate-layout "
                    f"crossmatch: {'; '.join(reasons)}."
                ),
                finding,
            )
    ...
    duplicate_physical_wells = sorted({
        f"{labware}:{well}"
        for labware, well in physical_destinations
        if physical_destinations.count((labware, well)) > 1
    })
    ...
    expected_plate_count = (
        (total_samples + pattern.samples_per_plate - 1)
        // pattern.samples_per_plate
        if total_samples
        else 0
    )
    ...
    plate_count_status = (
        "pass"
        if len(actual_plate_order) == expected_plate_count
        else "fail"
    )
    ...
    if plate_count_status == "fail":
        add_issue(
            ...
            (
                f"{total_samples} sample(s) with "
                f"{pattern.samples_per_plate} sample(s) per plate require "
                f"{expected_plate_count} destination plate(s), but the "
                f"trace resolved {len(actual_plate_order)} valid plate cycle(s)."
            ),
            {
                "expected_destination_plate_count": expected_plate_count,
                "actual_destination_plate_order": actual_plate_order,
    ...
    labware_findings, labware_issues = (
            ...
            for item in execution_plan:
                ...
                if (
                    ...
                    ph_inc_sequence(
                        ...
                        increment=int(
                            item.get("increment")
                            or count_active_channels(channel)
                        ),
                    )
                ...
                if action == "tip_pick":
                    ...
                    if mode == "head":
                        ...
                        # Channels use the VENUS sequence directly. Do not force
                        # a LayoutManager resource assignment that can fail needlessly.
                        ph_aspirate_seq(
                            ham_int,
                            asp_seq=seq,
                            vols=volume,
...
def build_report_sections(output: DevPalLiteOutput, report_path: str) -> Dict[str, Any]:
    ...
    for config in sorted(
        ...
        line = (
            f"Set {config.get('set_order', 1)}: {config.get('role')} - "
        ...
        if config.get("role") in {"aspirate", "dispense"}:
            line += (
    ...
    for marker_label, entries in sorted(
        ...
        marker_summary.append(
            f"{marker_label}: "
            + join_values([entry.get("dest_value") for entry in entries])
        )
    ...
    for finding in output.trace_findings:
        ...
        if ftype == "multi_plate_layout_summary":
            trace_summary.append(
                f"Multi-plate validation: {finding.get('total_samples_processed')} sample(s); "
                f"{finding.get('samples_per_plate')} sample(s)/plate; "
                f"expected {finding.get('expected_destination_plate_count')} plate(s); "
                f"actual plate order {join_values(finding.get('actual_destination_plate_order', []))}; "
                f"status {finding.get('status')}."
            )
        elif ftype == "trace_layout_match":
            trace_summary.append(
                f"Sample {finding.get('sample_number')} -> template "
                f"{finding.get('template_marker')} -> plate cycle "
                f"{finding.get('plate_cycle')}: expected "
                f"{join_values(finding.get('expected_destinations', []))}; actual "
                f"{join_values(finding.get('actual_destinations', []))}; labware "
                f"{join_values(finding.get('actual_destination_labware', []))}; "
                f"status {finding.get('status')}."
            )
        elif ftype == "expected_runtime_step_check":
            trace_summary.append(
                f"Runtime actions: planned "
                f"{finding.get('planned_liquid_handling_step_count')}; trace "
...
def write_docx_report(output: DevPalLiteOutput, output_dir: str) -> str:
    ...
    if sections["major_critical"]:
        for issue in sections["major_critical"]:
            p = doc.add_paragraph()
            run = p.add_run(
    ...
    for title, key in (
        ...
        if not lines:
            ...
            for line in lines:
                doc.add_paragraph(f"• {safe_text(line)}")
...
class DevPalLiteApp:
    ...
    def collect_configs(self) -> Optional[List[SequenceStepConfig]]:
        ...
        lay_metadata = parse_lay_sequence_metadata(
            self.lay_path
        )
        ...
        # Fixed: minor "tip type could not be confirmed" messages do not block.
        blocking_tip_issues = [
            issue
        ...
        if blocking_tip_issues:
            messagebox.showerror(
                "Tip Type / Volume Rule Error",
                "\n".join(
                    unique_issue_messages(
                        blocking_tip_issues
                    )[:10]
                ),
            )
    ...
    def generate_plan_run_and_validate(self):
        ...
        self.status.config(
            text="Running PyHamilton simulation..."
        )
